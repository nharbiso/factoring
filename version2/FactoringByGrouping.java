...
public class FactoringByGrouping
{
   ...
   */
   private static String factorByGrouping(Expression exp)
   {
      ...
      for(int[] pos : positions)
      {
         ...
         e1.addTerm(new Term(exp.getCoef(pos[0]), exp.getVars(pos[0])));
         e1.addTerm(new Term(exp.getCoef(pos[1]), exp.getVars(pos[1])));
      
         Expression e2 = new Expression();
         e2.addTerm(new Term(exp.getCoef(pos[2]), exp.getVars(pos[2])));
         e2.addTerm(new Term(exp.getCoef(pos[3]), exp.getVars(pos[3])));
         ...
         if(e1.equals(e2))      
            return FactoringBinomials.factor(e1, e1.getAllVar()) + FactoringBinomials.factor(fac, fac.getAllVar());
      }
      return exp.toString();
   }
   /**
   ...
   */
   private static String factorBySquares(Expression exp)
   {
      ...
      {
         if(!Functions.isPerfectSq(Math.abs(exp.getCoef(i))))
            continue loop;
         for(char var : exp.getVars(i).keySet())
            if(exp.getPow(i, var) % 2 != 0)
               continue loop;
         if(exp.getCoef(i) > 0)
            posSqInd.add(i);
         else
   ...
   */
   private static String factorSquares(Expression exp, int posInd, int negInd)
   {
      ...
      exp1.addTerm((int) Math.sqrt(exp.getCoef(posInd)), exp.getVars(posInd), new Fraction("1/2"));
      exp1.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(negInd))), exp.getVars(negInd), new Fraction("1/2"));
      Expression exp2 = new Expression();
      exp2.addTerm((int) Math.sqrt(exp.getCoef(posInd)), exp.getVars(posInd), new Fraction("1/2"));
      exp2.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(negInd))) * -1, exp.getVars(negInd), new Fraction("1/2"));
      Expression exp3 = new Expression();
      for(int i = 0; i < exp.size(); i++)
         if(i != posInd && i != negInd)
            exp3.addTerm(new Term(exp.getCoef(i), exp.getVars(i)));
      ...
      {
         exp2.addTerm(fac);
         return FactoringBinomials.factor(exp1, exp1.getAllVar()) + "(" + exp2.toString() + ")";
      }
      else if(exp3.equals(exp2))
      {
         exp1.addTerm(fac);
         return FactoringBinomials.factor(exp2, exp2.getAllVar()) + "(" + exp1.toString() + ")";
      }
      ...
      for(int x = 0; x < otherIndices.size(); x++)
      {
         ...
         if(sqrt != null && Functions.isPerfectSq(Math.abs(exp.getCoef(otherIndices.get(x)))))
         {
            Expression sqrt2 = sqrt.getCopy();
            sqrt.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(negInd))), exp.getVars(negInd), new Fraction("1/2"));
            sqrt2.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(negInd))) * -1, exp.getVars(negInd), new Fraction("1/2"));
      ...
      for(int x = 0; x < otherIndices.size(); x++)
      {
         ...
         if(sqrt != null && Functions.isPerfectSq(Math.abs(exp.getCoef(otherIndices.get(x)))))
         {
            exp1 = new Expression();
            exp1.addTerm((int) Math.sqrt(exp.getCoef(posInd)), exp.getVars(posInd), new Fraction("1/2"));
            exp1.addExpression(sqrt.getCopy());
            exp2 = new Expression();
            exp2.addTerm((int) Math.sqrt(exp.getCoef(posInd)), exp.getVars(posInd), new Fraction("1/2"));
            sqrt.multiply(-1);
   ...
   */
   private static Expression backwards(Expression exp)
   {
      ...
      e.addTerm(new Term(exp.getCoef(1), exp.getVars(1)));
      e.addTerm(new Term(exp.getCoef(0), exp.getVars(0)));
      return e;
   }
   /**
   ...
   */
   ...
   {
      if((int) Math.sqrt(Math.abs(exp.getCoef(a))) * (int) Math.sqrt(Math.abs(exp.getCoef(c))) * 2 != Math.abs(exp.getCoef(b)))
         return null;
      ...
      sqrt.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(a))), exp.getVars(a), new Fraction("1/2"));
      sqrt.addTerm((int) Math.sqrt(Math.abs(exp.getCoef(c))) * exp.getCoef(b) / Math.abs(exp.getCoef(b)) * exp.getCoef(a) / Math.abs(exp.getCoef(a)), exp.getVars(c), new Fraction("1/2"));
      return sqrt;
   }
}