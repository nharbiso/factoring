...
public class Factoring
{
   ...
   */
   public static String factor(String ex) throws Exception
   {
      ...
      if(exp.size() == 3 && canBeQuadFactored(exp, allVars))
         print = FactoringQuadratics.factor(exp);
      else if(exp.size() == 2)
         print = FactoringBinomials.factor(exp);
      else if(allVars.size() == 1)
      {
         exp.addZeroes();
         print = FactoringPolynomials.factor(exp);
      }
      else if(exp.size() == 4)
   ...
   */
   private static boolean canBeQuadFactored(Expression exp, Set<Character> allVars)
   {
      for(char c : allVars)
      {
         if(!(exp.getPower(0, c) == 0 && exp.getPower(1, c) == 0) && (double) exp.getPower(0, c) / 2 != exp.getPower(1, c) &&
            !(exp.getPower(2, c) == 0 && exp.getPower(1, c) == 0) && (double) exp.getPower(2, c) / 2 != exp.getPower(1, c))
            return false;
      }
      return true;
   }
